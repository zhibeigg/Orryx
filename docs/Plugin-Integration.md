...
- 服务端同一玩家同一时刻只保留一个 pending 瞄准请求，新请求会取消旧请求，默认 30 秒超时。
- `AimResponse`（ID `4`）必须返回当前请求的技能 ID，坐标、Yaw、Pitch 必须为有限数字，且目标位置不得超过服务端请求范围。
- 插件消息总长度上限为 32,766 字节；畸形、过期或越界响应会被拒绝，不会启动技能脚本。
- 当前 1.12.2 legacy 载荷顺序保持兼容：`[4:Int] [skillId:UTF] [x:Double] [y:Double] [z:Double] [yaw:Float] [pitch:Float]`。
