...
- 服务端同一玩家同一时刻只保留一个 pending 瞄准请求，新请求会取消旧请求；普通瞄准默认 30 秒超时，蓄力瞄准按 `maxTick` 加响应宽限期超时。
- 请求中的 `skillId` 是服务端为本次请求生成的一次性 wire token。客户端必须原样保存并在 `AimResponse`（ID `4`）中回传；不要自行替换成配置技能名。服务端完成校验后会恢复真实技能 ID。
- 客户端只有收到服务端 `AimConfirm=true` 后才能发送响应；未经确认、重复、跨会话重放或 token 不匹配的响应都会被拒绝。
- 坐标、Yaw、Pitch 必须为有限数字，目标位置不得超过 `AimRadiusAction` 定义的选区中心最大距离；指示图尺寸不会扩大该距离。
- 插件消息总长度上限为 32,766 字节；畸形、过期或越界响应不会启动技能脚本。
- 1.12.2 legacy 二进制载荷顺序保持不变：`[4:Int] [wireSkillId:UTF] [x:Double] [y:Double] [z:Double] [yaw:Float] [pitch:Float]`。
