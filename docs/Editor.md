...
填写 License 后插件会自动连接中心，不再需要额外开关。拥有 `Orryx.Command.Editor` 权限的玩家可执行 `/orryx edit`；该命令仅允许玩家执行，控制台不能签发玩家访问入口。
...
旧配置中的 `Editor.Enable` 与 `Editor.PublicUrl` 会被忽略。这样可以避免开关残留为 `false`、地址缺失或地址格式错误导致服务器永远不连接中心。License 为空时插件不会发起连接。
