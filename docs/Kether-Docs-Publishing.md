...
`actions-schema.json` 保留 `version: 2` 兼容旧客户端，并增加 `schemaVersion: 3`。v3 提供：

- Action、Selector、Trigger、Property 的稳定 ID。
- 完整 `syntax`、有序 inputs、examples、visibility。
- Action execution 和外部插件 requirements 提示。
- `properties` 数组。
- 插件版本和完整构建 commit。

Schema 本身不包含生成时间，确保同一源码可以确定性重建。时间只写入 Manifest。
