...
@Plugin("ArcartX")
object ArcartXKeyPressTrigger: AbstractPropertyEventTrigger<ClientKeyPressEvent>("ArcartX Key Press") {
    ...
    override val clazz
        get() = ClientKeyPressEvent::class.java

    override val specialKeys = arrayOf("Keys")

    override fun onJoin(event: ClientKeyPressEvent, map: Map<String, Any?>): ProxyCommandSender {
        return adaptPlayer(event.player)
    }

    override fun onCheck(station: IStation, event: ClientKeyPressEvent, map: Map<String, Any?>): Boolean {
        return super.onCheck(station, event, map) && ((map["Keys"] as? List<*>)?.contains(event.keyName) ?: (map["keys"] == event.keyName))
    }

    override fun onCheck(pipeTask: IPipeTask, event: ClientKeyPressEvent, map: Map<String, Any?>): Boolean {
        return pipeTask.scriptContext?.sender?.origin == event.player && ((map["Keys"] as? List<*>)?.contains(event.keyName) ?: (map["keys"] == event.keyName))
    }

    override fun read(instance: ClientKeyPressEvent, key: String): OpenResult {
        return when(key) {
    ...
    override fun write(instance: ClientKeyPressEvent, key: String, value: Any?): OpenResult {
