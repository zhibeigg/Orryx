...
object PlayerFishTrigger: AbstractPropertyPlayerEventTrigger<PlayerFishEvent>("Player Fish") {
    ...
    override val wiki: Trigger
        get() = Trigger.new(TriggerGroup.BUKKIT, event)
            ...
            .addParm(Type.TARGET, "caught", "玩家捕获的实体", nullable = true)
            .addParm(Type.INT, "expToDrop", "掉落的经验", writable = true)
            .addParm(Type.STRING, "hookState", "获取此鱼钩的当前状态：BOBBING/HOOKED_ENTITY/UNHOOKED")
            .addParm(Type.TARGET, "hookedEntity", "被钩中的实体")
            .addParm(Type.BOOLEAN, "applyLure", "获取是否应应用诱饵附魔来减少等待时间", writable = true)
            .addParm(Type.BOOLEAN, "isInOpenWater", "检查这个鱼钩是否在开阔的水域中")
            .addParm(Type.BOOLEAN, "isRainInfluenced", "等待和诱饵的时间是否会受到雨水的影响", writable = true)
            .addParm(Type.BOOLEAN, "isSkyInfluenced", "等待和诱饵的时间是否会受到直达天空的影响", writable = true)
            .addParm(Type.FLOAT, "maxLureAngle", "获取等待时间之后鱼出现的最大角度", writable = true)
            .addParm(Type.FLOAT, "minLureAngle", "获取等待时间之后鱼出现的最小角度", writable = true)
            .addParm(Type.INT, "maxLureTime", "获得鱼出现后等待鱼上钩所需等待的最大Tick数", writable = true)
            .addParm(Type.INT, "minLureTime", "获得鱼出现后等待鱼上钩所需等待的最小Tick数", writable = true)
            .addParm(Type.INT, "maxWaitTime", "获取等待鱼出现的最大Tick数", writable = true)
            .addParm(Type.INT, "minWaitTime", "获取等待鱼出现的最小Tick数", writable = true)
            .addParm(Type.DOUBLE, "biteChance", "咬钩几率", writable = true)
