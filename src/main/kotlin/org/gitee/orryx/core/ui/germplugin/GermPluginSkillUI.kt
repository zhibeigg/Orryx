...
open class GermPluginSkillUI(override val viewer: Player, override val owner: Player): AbstractSkillUI(viewer, owner) {
    ...
    protected open fun build(job: IPlayerJob): GermGuiScreen {
        ...
        val profile = owner.orryxProfile()
        ...
        skills.forEach { skill ->
            ...
            val pointCheck = skill.upgradePointCheck(skill.level, skill.level + 1)
            skillLevelUp.tooltip = listOf("&e等级 &f${skill.level}&7/&f${skill.skill.maxLevel}", "${if (pointCheck.second) "&a" else "&c"}右键!升级 &f消耗技能点 &e${pointCheck.first}", "&f总技能点 &e${profile.point}")
