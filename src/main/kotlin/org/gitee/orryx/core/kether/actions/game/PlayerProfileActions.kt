            ...
            case("count") {
                ...
                actionFuture { f ->
                    ensureSync {
                        containerOrSelf(they) { container ->
                            f.complete(
                                container.firstInstanceOrNull<PlayerTarget>()
                                    ?.let { target -> Orryx.api().profileAPI.superBodyCountdown(target.getSource()) / 50 })
                        }
                    }
                }
            }
                ...
                actionFuture { f ->
                    ensureSync {
                        containerOrSelf(they) { container ->
                            f.complete(
                                container.firstInstanceOrNull<PlayerTarget>()
                                    ?.let { target -> Orryx.api().profileAPI.superFootCountdown(target.getSource()) / 50 })
                        }
                    }
                }
            }
                ...
                actionFuture { f ->
                    ensureSync {
                        containerOrSelf(they) { container ->
                            f.complete(
                                container.firstInstanceOrNull<PlayerTarget>()
                                    ?.let { target -> Orryx.api().profileAPI.invincibleCountdown(target.getSource()) / 50 })
                        }
                    }
                }
            }
