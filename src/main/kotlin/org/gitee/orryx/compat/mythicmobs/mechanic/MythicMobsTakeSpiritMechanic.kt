...
@Ghost
class MythicMobsTakeSpiritMechanic(line: String, mlc: MythicLineConfig) : SkillMechanic(line, mlc), ITargetedEntitySkill {
    ...
    override fun castAtEntity(data: SkillMetadata, target: AbstractEntity): Boolean {
        return if (target.isPlayer) {
            ...
            ISpiritManager.INSTANCE.takeSpirit(player, amount.get(data))
            true
        } else {
            false
        }
