...
@Ghost
class MythicMobsSilenceMechanic(line: String, mlc: MythicLineConfig) : SkillMechanic(line, mlc), ITargetedEntitySkill {
    ...
    override fun castAtEntity(data: SkillMetadata, target: AbstractEntity): Boolean {
        return if (target.isPlayer) {
            ...
            Orryx.api().profileAPI.setSilence(player, duration.get(data).toLong())
            true
        } else {
            false
        }
